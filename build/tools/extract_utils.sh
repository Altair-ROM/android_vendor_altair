...
function setup_vendor() {
    ...
    export ALTAIR_ROOT="$3"
    if [ ! -d "$ALTAIR_ROOT" ]; then
        echo "\$ALTAIR_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$ALTAIR_ROOT/$OUTDIR" ]; then
        mkdir -p "$ALTAIR_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$ALTAIR_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$ALTAIR_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$ALTAIR_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$ALTAIR_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$ALTAIR_ROOT"/prebuilts/tools-lineage/common/smali/baksmali.jar
        export SMALIJAR="$ALTAIR_ROOT"/prebuilts/tools-lineage/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$ALTAIR_ROOT"/prebuilts/tools-lineage/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$ALTAIR_ROOT"/prebuilts/tools-lineage/${HOST}-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$ALTAIR_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$ALTAIR_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$ALTAIR_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$LINEAGE_ROOT"/vendor/altair/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$ALTAIR_ROOT"/"$OUTDIR"/radio
